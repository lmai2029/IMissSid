...
void TIMA0_PWM_init(uint8_t pin, uint32_t period, uint32_t prescaler, double percentDutyCycle){
	...
	if (!(TIMA0->GPRCM.PWREN & GPTIMER_PWREN_ENABLE_MASK)) {
		...
		switch(pin){
			case 0: // channels
				IOMUX->SECCFG.PINCM[IOMUX_PINCM25]|= ( IOMUX_PINCM_PC_CONNECTED | 0x04 ); // PF and PC set
				...
				break;
			case 1:
				IOMUX->SECCFG.PINCM[IOMUX_PINCM29]|= ( IOMUX_PINCM_PC_CONNECTED | 0x05 ); // PF and PC set
				...
				break;
			case 2:
				IOMUX->SECCFG.PINCM[IOMUX_PINCM43]|= ( IOMUX_PINCM_PC_CONNECTED | 0x06 ); // PF and PC set
				...
				TIMA0->COMMONREGS.ODIS |= GPTIMER_ODIS_C0CCP2_CCP_OUTPUT_OCTL;
				break;
			case 3:
				IOMUX->SECCFG.PINCM[IOMUX_PINCM30]|= ( IOMUX_PINCM_PC_CONNECTED | 0x03 ); // PF and PC set
				...
				|GPTIMER_OCTL_23_CCPIV_LOW);
				TIMA0->COMMONREGS.ODIS |= GPTIMER_ODIS_C0CCP3_CCP_OUTPUT_OCTL;
				break;
		}		
...
void TIMA1_PWM_init(uint8_t pin, uint32_t period, uint32_t prescaler, double percentDutyCycle){	 
	if (!(TIMA1->GPRCM.PWREN & GPTIMER_PWREN_ENABLE_MASK)) {
		...
		switch(pin){
			case 0:
				IOMUX->SECCFG.PINCM[IOMUX_PINCM17]|= ( IOMUX_PINCM_PC_CONNECTED | 0x04 ); // PF and PC set
