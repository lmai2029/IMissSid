...
void TIMG0_init(uint32_t period, uint32_t prescaler){
	if (!(TIMG0->GPRCM.PWREN & GPTIMER_PWREN_ENABLE_MASK)) {
        ...
        TIMG0->GPRCM.PWREN = (GPTIMER_RSTCTL_KEY_UNLOCK_W  | GPTIMER_PWREN_ENABLE_ENABLE);
    }
    
		uint32_t freq = SYSCTL_SYSCLK_getULPCLK();
		uint32_t timclk = freq/((period+1) * (prescaler+1));
...
void TIMG6_init(uint32_t period, uint32_t prescaler){
	if (!(TIMG6->GPRCM.PWREN & GPTIMER_PWREN_ENABLE_MASK)) {
        ...
        TIMG6->GPRCM.PWREN = (GPTIMER_RSTCTL_KEY_UNLOCK_W  | GPTIMER_PWREN_ENABLE_ENABLE);
    }
    ...
    TIMG6->COMMONREGS.CPS |= prescaler;
		...
		TIMG6->COUNTERREGS.CTRCTL |= GPTIMER_CTRCTL_EN_ENABLED|GPTIMER_CTRCTL_CM_DOWN| // enable timer
		GPTIMER_CTRCTL_CVAE_LDVAL|GPTIMER_CTRCTL_REPEAT_REPEAT_1;
		TIMG6->COUNTERREGS.LOAD = timclk;
...
void TIMG12_init(uint32_t period){
	if (!(TIMG12->GPRCM.PWREN & GPTIMER_PWREN_ENABLE_MASK)) {
        ...
        TIMG12->GPRCM.PWREN = (GPTIMER_RSTCTL_KEY_UNLOCK_W  | GPTIMER_PWREN_ENABLE_ENABLE);
    }
	
		uint32_t freq = SYSCTL_SYSCLK_getMCLK();
		uint32_t timclk = freq/(period+1);
		...
		TIMG12->COUNTERREGS.CTRCTL |= GPTIMER_CTRCTL_EN_ENABLED|GPTIMER_CTRCTL_CM_DOWN|
		GPTIMER_CTRCTL_CVAE_LDVAL|GPTIMER_CTRCTL_REPEAT_REPEAT_1;
		TIMG12->COUNTERREGS.LOAD = timclk;
